...
void Window::paintEvent(QPaintEvent * /* event */)
{
	...
	for(int k = 0; k < 2; k++) {
		if(k == 0) { x = a_scaled; }
        	...
        	if (is_first_method) {
			if(display != 3) { value =EvaluationAkima(x, a_scaled, b_scaled, n, X, A1); }
			else { value = fabs(EvaluationAkima(x, a_scaled, b_scaled, n, X, A1) - f(x)); }
			min_y = GetMin(min_y, value, first_p, 0);
			max_y = GetMax(max_y, value, first_p, 0);
	        }
        	if (is_second_method) {
			if(display != 3) { value = EvaluationSplines(x, a_scaled, b_scaled, n, X, A2); }
			else { value = fabs(EvaluationSplines(x, a_scaled, b_scaled, n, X, A2) - f(x)); }
			min_y = GetMin(min_y, value, first_p, 0);
			max_y = GetMax(max_y, value, first_p, 0);
		}
	}
                               ...
                               int n, const double *X, const double *A, int m)
{
	...
	double y2 = 0.0;
	QPen pen("black");
	if(m == 1) { pen= QColor("red"); }
	else { pen = QColor("blue"); }
        pen.setWidth(0);
        painter.setPen(pen);
	if(m == 1) {
